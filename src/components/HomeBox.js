      ...
      <div data-testid="canvases" className="p-24">
        ...
        <div className="grid grid-cols-4"> 
          {userCanvases.map((canvas) => (
            <Project image={PreviewImage} title={canvas.title} id={"/Canvas?id=" + canvas.fileID} owner={canvas.owner} date={canvas.accessedDate ? (canvas.accessedDate.toDate().toDateString()) : "Not Accessed"} />
          ))}
        </div>
      </div>
    </div>
